...
const RoomDetailsPage = () => {
  ...
  const acceptBooking = async () => {
    try {
      ...
      if (response.statusCode === 200) {
        alert("Successful booking"); // Show alert message
        setConfirmationCode(response.bookingConfirmationCode); // Set booking confirmation code
        setShowMessage(true); // Show message
        // Hide message and navigate to homepage after 0.5 minute
        setTimeout(() => {
  ...
  return (
    <div className="room-details-booking">
      {showMessage && (
        <div>
          <p className="booking-success-message">
            Booking successful! Please save your confirmation code below as it will only be shown for 30 seconds.
          </p>
      ...
      <h2>Room Details</h2>
      <br />
      <img src={`http://localhost:8080/${roomPhotoUrl}`} alt={roomType} className="room-details-image" />
          ...
          <ul className="booking-list">
            {bookings.map((booking, index) => (
              <li key={booking.id} className="booking-item">
                ...
                <span className="booking-text">Room Type: {roomType} </span>
      ...
      <div className="booking-info">
        ...
        <button className="go-back-button" onClick={() => { setShowDatePicker(false); navigate('/rooms'); }}>Go Back</button>
        {showDatePicker && (
